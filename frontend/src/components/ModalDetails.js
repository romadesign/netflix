	...
	return (
		<div className="bg-[#000000e0]  fixed overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 md:inset-0  md:h-full  sm:w-full md:w-full grid items-center rounded-lg m-auto  ">
			<div className="relative xs:w-[300px]  sm:w-[400px] md:w-[750px]  h-[100vh] m-auto grid items-center ">
				<div onMouseLeave={onMouse} className="relative bg-white rounded-lg shadow dark:bg-[#141414] shadow-lg shadow-gray-500/300 md:w-[750px] ">
					<div className="">
						<button
							onClick={() => { closeModal() }}
							type="button" className="absolute z-10 top-3 right-5 text-gray-400   rounded-[50px] text-sm ml-auto inline-flex items-center ">
							<svg aria-hidden="true" className="w-11 h-11 bg-[black] rounded-[50px] " fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" ></path></svg>
						...
						<div className="pb-3 text-center  ">
							...
							<div className="md:flex md:flex-row pl-10 pr-10 pt-7">
								<div className="basis-2/3 text-left">
									<h4 className="text-xl font-extrabold text-gray-500 dark:text-gray-200 text-left  pt-2">Acerca de Los {movie.title}</h4>
									<span className="text-xs  text-gray-400 dark:text-gray-450">
										Creada por: <p className="text-base  text-gray-500 dark:text-gray-200">{movie.director}</p>
									</span>
									<span className="text-xs  text-gray-400 dark:text-gray-450">
										Premios: <p className="text-base  text-gray-500 dark:text-gray-200">{movie.award}</p>
									</span>
									<span className="text-xs  text-gray-400 dark:text-gray-450">
										País: <p className="text-base  text-gray-500 dark:text-gray-200">{movie.country}</p>
									</span>
								</div>
								<div className="basis-2/3 text-left">
									<span className="text-xs  text-gray-400 dark:text-gray-450">
										Duración: <p className="text-base  text-gray-500 dark:text-gray-200">{movie.duration}</p>
									</span>
									<span className="text-xs  text-gray-400 dark:text-gray-450">
										Estreno: <p className="text-base  text-gray-500 dark:text-gray-200">{movie.premiere}</p>
									</span>
									<span className="text-xs  text-gray-400 dark:text-gray-450">
										Productora: <p className="text-base  text-gray-500 dark:text-gray-200">{movie.producer}</p>
									</span>
									<span className="text-xs  text-gray-400 dark:text-gray-450">
										Estudio: <p className="text-base  text-gray-500 dark:text-gray-200">{movie.studio}</p>
									</span>
								</div>
							</div>
						</div>
